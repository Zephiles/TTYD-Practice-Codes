...
void reloadRoomMain()
{
    ...
    copyStringAndNullTerminate(newBeroPtr, reloadCheatPtr->getNewBeroSize(), _next_bero);

    char *newMapPtr = reloadCheatPtr->getNewMapPtr();
    copyStringAndNullTerminate(newMapPtr, reloadCheatPtr->getNewMapSize(), _next_map);
