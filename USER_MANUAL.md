...
17. **Change Yoshi Color** allows you to change Yoshi's color at any time. You must swap partners for the new color to take effect.
18. **Disable D-Pad Options Display** prevents the D-Pad options display at the bottom-left of the screen from appearing. More specifically, this is the display that shows which D-Pad button opens which menu in the pause menu. These buttons can still be used, regardless of whether the display is disabled or not.
