...
3. **Mario's Coordinates** displays Mario's coordinates at the bottom-left of the screen, with the option to display the coordinates in either decimal or hexadecimal.
...
6. **Button Inputs** displays which buttons are pressed at the bottom-left of the screen.
...
18. **Palace Skip** displays various information about performing this skip. The display contains the following:

    * The main timer (labeled **PST**) is set to reset and start once you leave the pause menu, and pauses once you press X. You can also manually reset this timer by holding its button combination for two seconds.
    
    * **PhaEmy** keeps track of the Y coordinate for the phantom ember.
    
    * **ItemTimer** keeps track of the timer for the current item in the field.
    
    * **ParY** keeps track of your partner's Y coordinate.
...
20. **Arbitrary Memory Write** displays various information about performing this glitch. The display contains the following:

    * The pause timer (labeled **PT**) is set to reset and start once you leave the pause menu.
    
    * (Only available in the **Japanese** version due to the Spin Jump method of the glitch being possible in the **Japanese** version): The Spin Jump timer (labeled **SJT**) is set to reset upon jumping, start upon starting a Spin Jump, and stop upon first striking or entering a battle.
    
    * Both the **PT** and **SJT** values can be manually reset by holding its button combination for two seconds.'
    
    * **Pos X** and **Pos Z** displays Mario's current X and Z coordinates respectively in hexadecimal format.

* When the **Adjust X and Z Coordinates' Text Colors** option is enabled, the colors for the values specified by **Pos X** and **Pos Z** will change when Mario reaches key points in the **AMW** setup. Note that this functionality is currently only present in the **Japanese** version, as there are currently no setups for performing an **AMW** in the other versions.

* (Only available in the **Japanese** version due to the Spin Jump method of the glitch only being possible in the **Japanese** version): When the **Adjust X-Naut's Position** option is enabled, the X-Naut in the Great Tree room where the glitch is performed will be locked to the bottom-left of the room until Mario is standing at the required position, and is also not in Paper Mode nor in the pause menu. Upon this happening, there will be a delay of about one second before the X-Naut will be placed near Mario, and will be placed at a random Z coordinate.

* Note that a separate display exists that will always display text when any **AMW** is performed, in which it will state the Z coordinate that Mario was at and the memory address that it ends up writing to. This display will also prevent that write from actually occuring if it would end up writing to an invalid address, as this would end up causing the game to crash.

* When an **AMW** is being performed, a patch is applied to allow it to properly overwrite memory that would be overwritten under vanilla circumstances (as loading this mod into memory causes dynamic memory to start at a different memory address), and another patch is applied to remove the chance of the glitch randomly failing at the very end due to a check against a timer.

21. **Jabbi Hive Skip** displays various information about performing this skip. The display contains the following:

    * The first field tells you how well your pause jump was (pressed **D-Pad Left** first, pressed **A** first, or pressed both buttons at the same time).
    
    * **FBP** is the amount of frames between pressing **D-Pad Left** and **A** to do the unspin jump.
    
    * **FBUP** is the amount of frames between the two **A** presses for closing the menu.
    
    * All of the information in this display can be reset by pressing its button combination three times consecutively.

22. **Bridge Skip** displays various information about performing this skip. The display contains the following:

    * The first field in this display is how many frames early/late you were with pressing A. If you press A on the correct frame, then the counter will remain at 0. You can also manually reset this counter by holding its button combination for two seconds.
    
    * **HRP** is the pointer to the area of ground in the current room that you will respawn at upon touching a hazard. In order for **Bridge Skip** to work (as well as any other **Hazard Respawn Glitch**), this pointer must not be set, in which the value will be `0x00000000`.

* Note: **Bridge Skip** will be possible in the **PAL** version of the game when this display is active.

23. **Blimp Ticket Skip** displays various information about performing this skip. The display contains the following:

    * The up-right timer (labeled **URT**) is set to reset and start running once you leave the pause menu, and pauses once the held stick angle reaches 25 or higher on **JP**, and 27 or higher on **US**/**EU**.
    
    * The straight-up timer (labeled **SUT**) is set to reset and start running once you leave the pause menu, and pauses once the held stick angle is 0.00.
    
    * Both of these timers can be manually reset by holding its button combination for two seconds.
