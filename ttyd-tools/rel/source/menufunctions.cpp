...
void customTextInit(const char *initialText, uint32_t maxTextSize)
{
    char *tempBuffer = reinterpret_cast<char *>(
        clearMemory(CustomText.Buffer, CUSTOM_TEXT_BUFFER_SIZE));
    ...
    if (initialText)
    {
        ...
        else
        {
            ...
            CustomText.CurrentIndex = static_cast<uint8_t>(Length);
...
uint32_t deleteCustomState(uint32_t stateIndex)
{
    ...
    if (tempTotalEntries > 0)
    {
        ...
        CustomState.State = resizeStateMemory(tempState, tempTotalEntries, 0);
    }
    else
    {
